...
public class TokenProvider {
	...
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	   ...
	   public Map<String, Object> CreateToken(String id) {
        ...
        Redis.setrefresh(id, refresh_Token, refresh_time);
     ...
     //회원 정보 가져오기
	   public Map<String ,Object> kakao_validtoken(String token) {
		   logger.info("카카오 회원 정보 가져오기");
		   logger.info("jwt : " + token);
		   return Jwts.parser().setSigningKey(secretKey.getBytes()).parseClaimsJws(token).getBody(); 
	   }
	   //카카오 회원 access_token 만료 시간 체크
	   public boolean exp_validateToken(String token) {
		   logger.info("토큰 만료 시간 체크");
	        try {
	   ...
	   public ResponseEntity<Map<String ,Object>> access_check(HttpServletRequest request) {
			...
			exp_check =exp_validateToken(access_token);
			...
			result.put("resultmsg", "token is valide");
			
			return ResponseEntity.status(HttpStatus.OK).body(result);
		}

	   //access_token 조회
	   public Map<String , Object>  ValidToken(String Token , String id){
