...
import com.pets.platform.Entity.Content_heart;
import com.pets.platform.Entity.Content_info;
import com.pets.platform.Entity.Single_Comment;
import com.pets.platform.Entity.Upload;
...
public class Main_Service {
	...
	public Map<String ,Object> Small_Profile(String myid , String Userid){
		...
		logger.info("나의 아이디  :" + myid);
		logger.info("상대 닉네임 :" + Userid);
		input_data.put("Myid", myid);
		input_data.put("Userid", Userid);

		
			logger.info("토큰 시간 유효 ");
			logger.info("사용자 id :" + myid);
			
			saerch_data = mapper.friendfind(input_data);
			logger.info("사용자 정보 :" + saerch_data);
			if(!saerch_data.containsKey("profile")){
				logger.info("프로필 없다");
				saerch_data.put("profile" , "null");
			}
			
			files_info = mongo_template.find(Query.query(Criteria.where("user")
            		.is(Userid)).limit(3),Upload.class);
            logger.info("업로드한 파일 리스트 :" + files_info.size());
            
            List<String> url = new ArrayList<String>();
            if(files_info.size()!= 0) {
            	 for(int i=0;i<files_info.size();i++) {
                 	logger.info("i :" + i);
                 	file_list = files_info.get(i).getFileinfo();
                 	logger.info("1111 :" );
                 	logger.info("파일 리스트 :" + file_list.get(0).get(""));
                 	if(file_list.get(0).get("type").equals("video/mp4")) {
                 		logger.info("비디오 파일 ");
                 		//url.add(file_list.get(0).get("Path").toString());
                 		
                 		
                 	}
                 	else{
                 		//url.add(file_list.get(0).get("url").toString());
                 	}

                 	
                 }
                 total_data.put("url", url);
            ...
            data.put("resultdata", total_data);
	...
	public Map<String, Object> Profile(String id , String type , String Userid){
		...
		List<Map<String, Object>> file_list = new ArrayList<Map<String, Object>>();
		//List<Content_info> content = new ArrayList<Content_info>();
		...
		if(type.equals("My")) {
            ...
            files_info = mongo_template.find(Query.query(Criteria.where("user").is(id)),Upload.class);
            logger.info("업로드한 파일 리스트 :" + files_info);

            
            for(int i=0;i<files_info.size();i++) {
                Map<String, Object> content_info= new HashMap<String, Object>();
        		List<Single_Comment> comment = new ArrayList<Single_Comment>();
        		List<Content_heart> heart = new ArrayList<Content_heart>();
            	String uuid="";
			...
			logger.info("다른 페이지 조회");
			boolean check_private = false;
			logger.info("user_id :" + Userid);
			check_private = setting_mapper.search_private(Userid);
			 logger.info("공개 여부 :" +check_private);
				Map<String , Object> user_data = new HashMap<String,Object>();
				user_data = mapper.user_info(Userid);
				logger.info("사용자 정보 :" + user_data);
				...
				if(check_private == true) {
					...
					Fl_info=mapper.friendfind(userinfo);
		            if(!user_data.containsKey("profile_img") && !user_data.containsKey("thumnail_img")) {
		            	user_data.put("profile_img", "null");
		            }
		            response_data.put("id", user_data.get("id").toString());
					...
					userinfo.put("Userid", Userid);
					Fl_info=mapper.friendfind(userinfo);
					logger.info("팔로워 및 정보 :" +Fl_info);
		            files_info = mongo_template.find(Query.query(Criteria.where("user").is(Userid)),Upload.class);
		            logger.info("업로드한 파일 리스트 :" + files_info);
		            for(int i=0;i<files_info.size();i++) {
		                Map<String, Object> content_info= new HashMap<String, Object>();
		        		List<Single_Comment> comment = new ArrayList<Single_Comment>();
...
*/
	...
	public Map<String, Object> Mainpage_refresh(/*HttpServletRequest access_token,*/ String id) {
		Map<String, Object>result_data = new HashMap<String, Object>();
		Map<String, Object>response_data = new HashMap<String, Object>();
		Map<String, Object>token_data = new HashMap<String, Object>();
		...
		public_content = mongo_template.find(Query.query(Criteria.where("user").ne(id)),Upload.class);
		logger.info("publci content :" + public_content);
		...
		if(public_content.size() == 0) {
	        ...
	        result_data.put("resultmsg", "file is empty");
	        result_data.put("resultdata", response_data);
			result_data.put("resultcode", 201);
			return result_data;
		}
			...
			for(int i=0; i<public_content.size();i++) {
				logger.info("리스트 데이터 출력");
				//file= list.get(i).getFileinfo();
				//int favorite = public_content.get(i).getFavorite();
				String userid = "";
				userid =public_content.get(i).getUser(); 
				String fix_id = public_content.get(i).getId();
				logger.info("사용자 id :" + fix_id);
				files_info = public_content.get(i).getFileinfo();
				Map<String, Object> content_info= new HashMap<String, Object>();
				List<Map<String, Object>> type_url = new ArrayList<Map<String, Object>>();
				...
				content = mongo_template.find(Query.query(Criteria.where("_id").is(fix_id)), Content_info.class);
				
				
				comment =mongo_template.find(Query.query(Criteria.where("_id").is(fix_id)), Single_Comment.class);
				
				if(comment.size() !=0) {
					content_info.put("comment", comment);
					logger.info("모든 댓글 가져오기");
				}
				else {
					logger.info("댓글이 없습니다.");
					content_info.put("comment", "null");
				}
				total_content.add(content_info);
				
				logger.info("게시물 데이터 :" + total_content);
		...
		//String access="";
		//access =access_token.getHeader("Authorization").toString(); 
		
		//boolean exp_check = false;
		//exp_check = token.exp_validateToken(access);
		...
		/*
            ...
            //token_data = token.kakao_validtoken(access);
            if(!user_data.containsKey("profile_img") && !user_data.containsKey("thumnail_img")) {
            	user_data.put("profile_img", "null");
            	//user_data.put("thumnail_img", "null");
            }
	...
	/*
			...
			if(check_refresh == null) {
				...
				if(!user_info.containsKey("profile_img")){
